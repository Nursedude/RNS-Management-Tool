...
# NOTE: RNODE helper functions and configure_rnode_interactive() live in lib/rnode.sh
# (single source of truth — previously duplicated here, removed in Session 14 / R5)
...
install_meshchat() {
    ...
    run_with_timeout "$NETWORK_TIMEOUT" npm audit fix --audit-level=moderate 2>&1 | tee -a "$UPDATE_LOG" || true
    next_step "success"
